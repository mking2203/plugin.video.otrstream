...
def setup_recording(params):
  ...
  #resultData = _otrDB_.zapi.exec_zapiCall('/zapi/playlist/program', params)
...
# 		 self.notification = None
		...
		self.viewStartDate -= datetime.timedelta(minutes=self.viewStartDate.minute % 30, seconds=self.viewStartDate.second)
                xbmcgui.Window(10000).setProperty('otrstream_runningView',"epg")
	...
	def close(self):
                ...
                xbmcgui.Window(10000).setProperty('otrstream_runningView',"")
	...
	def onInit(self):
		self.db = otrDB()
		if(__addon__.getSetting('loadChannels') == 'true'):
                    __addon__.setSetting('loadChannels','false')
		    self.db.updateChannels(rebuild=True)
                else:
                    self.db.updateChannels()
	...
	def onClick(self, controlId):
		...
		if SWISS == 'true':
			...
			if start > now :
				...
				print 'SERIES:  ' + str(_otrDB_.getSeries(program['showID']))
				if _otrDB_.getSeries(program['showID']):#Series record avilable
...
# 			self.setControlImage(self.C_MAIN_BACKGROUND, program['image_large'])
	...
	def _showDescription(self, id):		        
		description = otrDB().getShowInfo(id,'description')
